...
public class Parser {
    ...
    private Decl decl() {
	   ...
	   if (token == Token.LBRACKET) {
		   ...
		   match(Token.LBRACKET);   // "[" 검사
		   Value size = literal();  // n 을 받아와야 하기 때문에,  size를 literal로 받아옴
		   match(Token.RBRACKET);  // "]' 를 검사함
		   d = new Decl(id,t,size.intValue()); // id, t, size(int)를 넣어 array로 decl함
    ...
    // <assignment> -> id = <expr>;  
        ...
        if (token == Token.LBRACKET) {  // id[<expr>] = <expr>;
        	...
        	// Set the AST of Array(Identifier s, Expr e) to Array ar
           	match(Token.LBRACKET); // "[" 검사
        	Expr expr1 = expr();  // id[expr] 중  expr을 받아옴
        	match(Token.RBRACKET); // "]" 검사
        	ar = new Array(id,expr1); // Array로 저장
        }
        ...
        case ID:
            ...
            if (token == Token.LBRACKET) {	// id[<expr>]
            	...
            	// Set the AST of Array(Identifier s, Expr e) to Expr e
             	match(Token.LBRACKET);  // "[" 검사
            	Expr e1 = expr();       //  expr 검사
            	match(Token.RBRACKET);  // "]" 검사
            	e = new Array(v,e1);    // expr와 id 를 묶어 array로 저장함
            }
            break;
