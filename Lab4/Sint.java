...
public class Sint {
    ...
    State Eval(Assignment a, State state) {
        ...
        if (a.ar == null)
        	...
        	Identifier id = a.ar.id;         // assignment에서 정의된 ar의 id를 가지고 옴
        	Value index = V(a.ar.expr, state);  // ar.expr (index) 값을 가지고 옴
        	state.get(id).arrValue()[index.intValue()] = v; // ar[index] = v를 실행함. 위에서 만든 id, ar를 arrValue, intValue사용해 가져옴
        	return state;  // state 리턴
        }
    }
        ...
        if (ds != null) {
        	for (Decl decl : ds) 
			    if (decl.arraysize > 0) { // <type> id[n];
			    	...
			    	Value[] array = new Value[decl.arraysize]; // arraysize 크기의 Value 배열을 만듦
			    	state.push(decl.id, new Value(array)); // 만들어진 Value 배열을 state에 id와 함께 삽입함.
	            }
    ...
    Value V(Expr e, State state) {
        ...
        if (e instanceof Array) { // id[<expr>]
        	...
        	//     Value[] vs = v.arrValue();
        	//     return (vs[index])
        	Array ar = (Array) e;     // expr이 Array일 경우이기에 Array로 변경함
        	Value index = V(ar.expr,state); // ar.expr을 가져와 index를 가져옴
        	Value v = (Value)state.get(ar.id); // 또한 id와 맞는 value를 가져오고
         	Value[] vs = v.arrValue();   // array로 만듦
        	return vs[index.intValue()]; // 만들어진 array의 index 값을 리턴함
	    }
