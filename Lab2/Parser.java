...
*/
    ...
    {
    // <readStmt> -> read id;  
    	match(Token.READ);   // 명령어 read match 후 다음 토큰
    	Identifier id = new Identifier(match(Token.ID)); // id 가져오기
    	match(Token.SEMICOLON); // 세미콜론 확인
    	return new Read(id);  // 리턴
	// return null;
    }
    ...
    // <printStmt> -> print <expr>;
    	match(Token.PRINT);  // 명령어 print match
    	Expr e = expr();  // exper 가져오기
    	match(Token.SEMICOLON); // 세미콜론 확인 
    	return new Print(e); // 리턴
	//return null;
    }
...
*/
    ...
    // <whileStmt> -> while (<expr>) <stmt>
    	match(Token.WHILE);  // while match하기
    	match(Token.LPAREN);  // 소괄호 ( 확인
    	Expr e = expr();  // expr match하기 
    	match(Token.RPAREN); // 소괄호 ) 확인 
    	Stmt s = stmt();  // 조건문 끝 후 stmt 가져오기
    	
        return new While(e,s);  // 리턴
    }
    ...
    // <expr> -> <bexp> {& <bexp> | '|'<bexp>} | !<expr> | true | false
        ...
        if(token == Token.AND)  // token 이 and이면
        {
        	Operator op = new Operator(match(Token.AND)); //operator 로 and 가져오기
			return new Binary(op,e,bexp()); // e, bexp 를 op한 Binary 리턴
        }
        else if(token == Token.OR) // token이 or이면
        {
        	Operator op = new Operator(match(Token.OR)); // operator OR 가져오기
			return new Binary(op,e,bexp()); // e, bexp, op로 Binary 리턴
        }
        return e; // 예외
    }
    ...
    private Expr bexp() {
        // <bexp> -> <aexp> [ (< | <= | > | >= | == | !=) <aexp> ]
        Expr e = aexp();
        if(token == Token.LT) // "<"처리 aexp1, aexp2 비교한 값을 리턴  
        {
        	Operator op = new Operator(match(Token.LT));
        	Expr e2 = aexp();
        	return new Binary(op, e,e2);
        }
        else if(token == Token.LTEQ)// "<="처리 aexp1, aexp2 비교한 값을 리턴 
        {
        	Operator op = new Operator(match(Token.LTEQ));
        	Expr e2 = aexp();
        	return new Binary(op, e,e2);
        }
        else if(token == Token.GT) // ">"처리 aexp1, aexp2 비교한 값을 리턴 
        {
        	Operator op = new Operator(match(Token.GT));
        	Expr e2 = aexp();
        	return new Binary(op, e,e2);
        }
        else if(token == Token.GTEQ) // ">="처리 aexp1, aexp2 비교한 값을 리턴 
        {
        	Operator op = new Operator(match(Token.GTEQ));
        	Expr e2 = aexp();
        	return new Binary(op, e,e2);
        }
        else if(token == Token.EQUAL) // "=="처리 aexp1, aexp2 비교한 값을 리턴 
        {
        	Operator op = new Operator(match(Token.EQUAL));
        	Expr e2 = aexp();
        	return new Binary(op, e,e2);
        }
        else if(token == Token.NOTEQ) // "!="처리 aexp1, aexp2 비교한 값을 리턴 
        {
        	Operator op = new Operator(match(Token.NOTEQ));
        	Expr e2 = aexp();
        	return new Binary(op, e,e2);
        }
        else
