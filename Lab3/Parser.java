...
public class Parser {
    ...
    private Stmts doStmt() {
    	...
    	// student exercise
    	match(Token.DO); // do 검사
    	Stmts ss = new Stmts(); // 한번 이상이기에 <stmts> 만들기 
    	Stmt s = stmt(); //  다음 <stmt>를 저장후
    	ss.stmts.add(s); // stmts에 더하기 
    	match(Token.WHILE); // while 검사
    	match(Token.LPAREN); // ( 검사
        Expr e = expr(); // expr 검사
        match(Token.RPAREN); // ) 검사
        match(Token.SEMICOLON); // ; 검사

        ss.stmts.add(new While(e,s)); // 저장한 stmt를 while으로 다시 저장하기
    ...
    private Let forStmt() {
    	...
    	// student exercise
    	match(Token.FOR);  // for 검사
    	match(Token.LPAREN); // ( 검사
    	Decls ds = decls();  // <type> id = <expr>; 검사
    	Expr e1 = expr();   // <expr>
    	match(Token.SEMICOLON); // ; 검사
    	Identifier id = new Identifier(match(Token.ID)); // id 검사
    	match(Token.ASSIGN); // = 검사
    	Expr e2 = expr(); // <expr> 
    	Stmt assignStmt = new Assignment(id,e2); // assignment stmt 저장하기 
    	match(Token.RPAREN);  // ) 검사
    	Stmts s = new Stmts(stmt()); // stmts에 <stmt> 저장하기
    	s.stmts.add(assignStmt); // 그 이후 저장했던 assignment stmt 추가
    	Stmts ss = new Stmts(new While(e1,s));  // 저장했던 stmt를 while 로 다시 저장하기
    	
    	return new Let(ds,ss); // Let 문으로 리턴
    }
