...
public class Sint {
        ...
        if (ds != null) {
            for (Decl decl : ds) {
                ...
                if (decl.expr == null) {
                    ...
                    state.push(id, new Value(Type.INT)); // state에 id, defalt Value를 삽입함
                } else {
                    // TODO: [push a new variable with id and value of V(expr)]
                    state.push(id, V(decl.expr, state)); // state에 id와 V(expr)을 삽입함
                }
            }
            return state;
        }
    ...
    Value binaryOperation(Operator op, Value v1, Value v2) { // Lab03
        ...
        case "<":
        	if(v1.type() == Type.INT && v2.type() == Type.INT) // int일 때
        	{
        		return new Value(v1.intValue() < v2.intValue()); // intValue를 사용해 < 연산 후 값 리턴
        	}
        	else if(v1.type() == Type.STRING && v2.type() == Type.STRING) // string 일떄
        	{
        		return new Value(v1.stringValue().compareTo(v2.stringValue()) < 0); // CompareTo함수 사용해 < 연산
        	}
        case "<=":
        	if(v1.type() == Type.INT && v2.type() == Type.INT) // int 일 때
        	{
        		return new Value(v1.intValue() <= v2.intValue()); // intValue 사용해 <= 연산 후 리턴
        	}
        	else if(v1.type() == Type.STRING && v2.type() == Type.STRING)// string 일떄
        	{
        		return new Value(v1.stringValue().compareTo(v2.stringValue()) <= 0); // CompareTo함수 사용해 <= 연산
        ...
        case "&":
            return new Value(v1.boolValue() & v2.boolValue()); // boolValue 후 & 연산
        case "|":
            return new Value(v1.boolValue() | v2.boolValue()); // boolValue 후 | 연산
